...
public class StripeService {
					...
					.addLineItem(
							...
							try {
								...
								return session.getId();
									...
									System.out.println("Stripは、提供された情報では認証できません。");
									return "";
								}
									...
									System.out.println("Stripe側で問題が発生しました (稀な状態です)。");
									return "";
								}
	...
	public void processSessionCompleted(Event event) {
		...
		optionalStripeObject.ifPresentOrElse(stripeObject -> {
			...
			//StripeObjectオブジェクトをSessionオブジェクトに型を変換する
			...
			try {
				...
				Map<String, String> sessionMetadata = session.getPaymentIntentObject().
													  getMetadata();
				...
				System.out.println("短期間のうちに過剰な回数のAPIコールが行われました。");
			}
				...
				System.out.println("Stripsは、提供された情報では認証できません。");
			}
				...
				System.out.println("Srripe側で問題が発生しました(稀な状況です)。");
			}
			
			catch(StripeException e){
				System.out.println("Stripsとの通信中に予期せぬエラーが発生しました。");
			}
		...
		System.out.println("stripe-java Version;" + Stripe.VERSION + 
						   ", stripe-java API Version: " + Stripe.API_VERSION);
	}
